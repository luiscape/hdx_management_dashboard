...
function generateSparkline(table_name, indicator_name, div_id, verbose) {
  ...
  d3.json(url, function(error, json) {
    ...
    values = data.query().filter().values(indicator_name);
    dates = data.query().filter().values('date');
...
function buildHDXResourceUrl(id) {
...
generateSparkline(url, 'Total Persons of Concern', '#total-persons', verbose = true);